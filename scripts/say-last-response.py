...
Backs the `/say` slash command. You listen to long responses instead of
reading them; this removes the copy-paste-into-a-TTS-app step. Manual trigger
only — nothing auto-fires.
...
What it does:
  ...
  5. Write the mp3 to a STABLE path and print the play command. It does NOT
     auto-play: you run the player yourself so you get a keyboard for
     speed/seek/pause controls (a detached player can take no key presses).
     Same stable file is what makes over-SSH playback work — pull it to a
     local machine and play there. (controls + ssh are one design.)

API key: read from $OPENAI_API_KEY, else ~/.config/claude-say/openai-key. No
key is a clean one-line error, not a traceback.
...
Stdlib only (urllib for HTTP). Playback is your own mpv/ffplay run.
...
CONFIG_DIR = Path.home() / ".config" / "claude-say"
KEY_FILE = CONFIG_DIR / "openai-key"
PID_FILE = CONFIG_DIR / ".say-playing.pid"
# Stable output path (NOT a random tempfile). You play this file yourself in
# your own terminal so you get a keyboard for speed/seek/pause controls — a
# player Claude launches detached can never take key presses. The stable path
# is also what makes over-SSH playback work: pull THIS file to a local machine
# and play it there. (controls + ssh are one design.)
SAY_MP3 = CONFIG_DIR / "say-last.mp3"

TTS_URL = "https://api.openai.com/v1/audio/speech"
DEFAULT_MODEL = os.environ.get("CLAUDE_SAY_TTS_MODEL", "gpt-4o-mini-tts")
DEFAULT_VOICE = os.environ.get("CLAUDE_SAY_TTS_VOICE", "alloy")
...
def ssh_server_ip():
    ...
    SSH_CONNECTION is "client_ip client_port server_ip server_port"; a local
    machine reaches THIS host at server_ip to pull the audio.
    """
...
def play_instructions(minutes):
    """Build the copy-paste play commands (you launch the player).

    No auto-play. You run the player yourself so you get a keyboard for the
    speed/seek/pause controls. mpv is the only common player that does all
    three; ffplay is offered as a no-speed fallback if mpv is missing.
    """
    ...
    if server:
        lines.append(
            f"  local audio over ssh: ssh {server} 'cat {SAY_MP3}' | mpv -"
        )
...
def stop_playback():
    ...
    There is no detached player (you run mpv/ffplay in your own terminal and
    quit with q), so this is a best-effort cleanup of leftovers.
    """
    pid = None
    if PID_FILE.is_file():
        try:
